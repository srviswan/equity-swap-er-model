...
IF NOT EXISTS (SELECT 1 FROM sys.columns 
               WHERE object_id = OBJECT_ID('Trade') AND name = 'activity_quantity')
BEGIN
    ALTER TABLE Trade 
    ADD activity_quantity DECIMAL(18,6) NOT NULL DEFAULT 0,
        activity_notional DECIMAL(18,2) NOT NULL DEFAULT 0,
        settlement_date DATE,
        settlement_amount DECIMAL(18,2) DEFAULT 0,
        direction VARCHAR(10) CHECK (direction IN ('BUY', 'SELL', 'LONG', 'SHORT'));
END
GO
...
IF NOT EXISTS (SELECT 1 FROM sys.objects WHERE object_id = OBJECT_ID('ResetEvent'))
BEGIN
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_reset_trade_date')
    CREATE INDEX idx_reset_trade_date ON ResetEvent(trade_id, reset_date);

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_reset_date')
    CREATE INDEX idx_reset_date ON ResetEvent(reset_date);
GO
...
IF OBJECT_ID('trade_event_timeline', 'V') IS NOT NULL
    DROP VIEW trade_event_timeline;
GO

-- Create a comprehensive event view combining trades, settlements, and resets
CREATE VIEW trade_event_timeline AS
WITH 
...
-- Drop existing view if it exists
IF OBJECT_ID('aggregated_trade_events', 'V') IS NOT NULL
    DROP VIEW aggregated_trade_events;
GO

CREATE VIEW aggregated_trade_events AS
...
-- Drop existing view if it exists
IF OBJECT_ID('running_notional_calculation', 'V') IS NOT NULL
    DROP VIEW running_notional_calculation;
GO

CREATE VIEW running_notional_calculation AS
WITH 
...
latest_reset_prices AS (
    SELECT 
        ...
        a.event_date,
        COALESCE(
            -- Use OUTER APPLY to get latest reset price
            (
                SELECT TOP 1 r.reset_price
                FROM ResetEvent r
                WHERE r.trade_id = a.trade_id 
                  AND r.reset_date <= a.event_date
                ORDER BY r.reset_date DESC
            ),
            -- Default to initial price from first trade if no reset
            (
                SELECT TOP 1 t.activity_notional / NULLIF(t.activity_quantity, 0)
                FROM Trade t 
...
running_totals AS (
    SELECT 
        ROW_NUMBER() OVER (ORDER BY a.trade_id, a.event_date, a.event_priority, a.first_created) AS sequence_id,
        a.trade_id,
        a.event_date,
        a.event_type,
        a.total_quantity,
...
-- Drop existing view if it exists
IF OBJECT_ID('equity_swap_time_series', 'V') IS NOT NULL
    DROP VIEW equity_swap_time_series;
GO

CREATE VIEW equity_swap_time_series AS
SELECT 
    trade_id,
    event_date,
    event_type,
    ...
    DATEDIFF(DAY, LAG(event_date) OVER (PARTITION BY trade_id ORDER BY event_date), event_date) AS days_since_last_event,
    first_created
...
-- Drop existing view if it exists
IF OBJECT_ID('latest_trade_positions', 'V') IS NOT NULL
    DROP VIEW latest_trade_positions;
GO

CREATE VIEW latest_trade_positions AS
...
-- =============================================================================
-- STORED PROCEDURES - T-SQL equivalents for utility functions
-- =============================================================================

-- Stored procedure to get running position for a specific trade on a specific date
IF OBJECT_ID('get_trade_position', 'P') IS NOT NULL
    DROP PROCEDURE get_trade_position;
GO

CREATE PROCEDURE get_trade_position
...
-- Stored procedure to get time series for a date range
IF OBJECT_ID('get_trade_time_series', 'P') IS NOT NULL
    DROP PROCEDURE get_trade_time_series;
GO

CREATE PROCEDURE get_trade_time_series
    @p_trade_id VARCHAR(50),
...
BEGIN
    ...
    SELECT 
        ROW_NUMBER() OVER (ORDER BY event_date) AS sequence_num,
        event_date,
        event_type,
        event_quantity,
        event_notional,
...
IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_trade_activity')
    CREATE INDEX idx_trade_activity ON Trade(activity_quantity, activity_notional, settlement_date);

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_reset_event_lookup')
    CREATE INDEX idx_reset_event_lookup ON ResetEvent(trade_id, reset_date);

IF NOT EXISTS (SELECT 1 FROM sys.indexes WHERE name = 'idx_settlement_status_date')
    CREATE INDEX idx_settlement_status_date ON Settlement(settlement_status, settlement_date);
GO
...
-- Drop existing view if it exists
IF OBJECT_ID('calculation_validation', 'V') IS NOT NULL
    DROP VIEW calculation_validation;
GO

CREATE VIEW calculation_validation AS
SELECT 
    trade_id,
